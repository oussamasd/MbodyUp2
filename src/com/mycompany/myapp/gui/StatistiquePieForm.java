...
public class StatistiquePieForm extends BaseForm {
    ...
    private double nbr_reclamation = 70;
    private HashMap<String , Double>CatList ;
    
    Form current;
BaseForm form;
        public StatistiquePieForm(Resources res , HashMap<String , Double>c )  {
       ...
       // addTab(swipe, res.getImage("news-item-2.jpeg"), spacer1, "Bienvenue");
...
//        rbs[0].setSelected(true);
  ...
  ;       
    ...
    SimpleSeriesRenderer simpleSerieRenderer=new SimpleSeriesRenderer();
    simpleSerieRenderer.setColor(color);
    renderer.addSeriesRenderer(simpleSerieRenderer);
    }
    return renderer;
    }
    public void createPieChartForm(){
        ...
        double total=0;
        for(Double k : CatList.values()){
            total+=k;
        }
       for(Map.Entry<String , Double> a : CatList.entrySet()){
       a.setValue((a.getValue()*100)/total);
        ...
        int[] colors=new int[CatList.size()];//{,0x52b29a,0x52879a}
        for(int hh=0 ; hh<CatList.size();hh++){
            colors[hh]=16030674+(1000*hh) ;
        }
        ...
        PieChart chart = new PieChart(buildDataset("title",Math.round(prcntfeed) ,Math.round(prcntrec),CatList),renderer);
    ...
    private CategorySeries buildDataset(String title ,double prcntfeed , double prcntrec,HashMap<String,Double> cc){
        CategorySeries series = new CategorySeries(title);
        for(Map.Entry<String,Double> o : cc.entrySet()){
             series.add(o.getKey(),Math.round(o.getValue()) );
            
        }
        //series.add("Recla",prcntrec);
        //series.add("feed" , prcntfeed);
        return series;
