...
public class ListCategoriesForm extends BaseForm{
    ...
    public ListCategoriesForm(Resources res)
    {
        ...
        setTitle("Ajout Type categories");
       ...
       addTab(swipe,s1,res.getImage("no.jpg"),"","",res);
        ...
        RadioButton mesListes = RadioButton.createToggle("Ajouter Categories", barGroup);
        mesListes.setUIID("SelectBar");
        RadioButton liste = RadioButton.createToggle("Modifier Categories", barGroup);
        liste.setUIID("SelectBar");
        RadioButton partage = RadioButton.createToggle("Mes Categories ", barGroup);
        partage.setUIID("SelectBar");
        Label arrow = new Label(res.getImage("news-tab-down-arrow.png"), "Container");


        mesListes.addActionListener((e) -> {
               new AjoutCategoriesForm(res).show();
        });
         liste.addActionListener((e) -> {
                new ModifierCategoriesForm(res,rec).show();
        });
        ...
        ArrayList<Categories>list = ServiceCategories.getInstance().affichageCategorie();
        for(Categories rec : list)
        {
    ...
    private void addButton(Image img, Categories rec,Resources res) {
...
////butt supprimer
        ...
        lSupprimer.addPointerPressedListener(l->{
            ...
            if(dig.show("Supression","Vous Voulez supprimer cette categories ?","Annuler","oui"))
            {
                dig.dispose();
            }
            else{
                dig.dispose();
                if(ServiceCategories.getInstance().deleteCategorie(rec.getId()))
                {
                    new ListCategoriesForm(res).show();
                }
            }
        });
